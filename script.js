onload = function() {
	...
	function createData() {
		cities = ['Agra', 'Delhi', 'Mumbai', 'Lucknow', 'Goa', 'Jammu', 'Kashi', 'Jaipur', 'Hyderabad', 'Bangalore',
			'Mizoram', 'Gujrat', 'Gangtok'
		];
        ...
        for(let i=2;i<=V;i++){
            ...
            edges.push({type: 0, from: i, to: neigh, color: 'red',label: String(Math.floor(Math.random()*70)+31)});
        }
        ...
        src = Math.floor(Math.random()*5)%13;
        dst = V;
        if(src==0)src=1;
        // if(src===dst){
        //     src=1;
